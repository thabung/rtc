...
function handleWindowResize() {
    ...
    connectCount = thunderrtc.getConnectionCount();
...
function killActiveBox() {
    if( activeBox > 0) {
        var thunderrtcid = thunderrtc.getIthCaller(activeBox-1);
        collapseToThumb();
        setTimeout( function() {
            thunderrtc.hangup(thunderrtcid);
            thunderrtc.sendServerMessage(null, "hangup", {hangupThunderrtcid:caller});
        }, 400);
    }
}
...
function callEverybodyElse(roomName, otherPeople) {
    ...
    thunderrtc.setRoomOccupantListener(null, null); // so we're only called once.
    ...
    function establishConnection(position) {
        ...
        thunderrtc.call(list[position], callSuccess, callFailure);
...
function sendText(e) {
    ...
    if( stringToSend && stringToSend !== "") {
        for(var i = 0; i < maxCALLERS; i++ ) {
            var thunderrtcid = thunderrtc.getIthCaller(i);
            if( thunderrtcid && thunderrtcid !== "") {
                thunderrtc.sendPeerMessage(thunderrtcid, "img", stringToSend);
            }
        }
    }
    return false;
...
function messageListener(thunderrtcid, msgType, content) {
    for(var i = 0; i < maxCALLERS; i++) {
        if( thunderrtc.getIthCaller(i) === thunderrtcid) {
...
function appInit() {
    ...
    // thunderrtc.setVideoBandwidth(20);
    thunderrtc.setRoomOccupantListener(callEverybodyElse);
    thunderrtc.easyApp("thunderrtc.room", "box0", ["box1", "box2", "box3"], loginSuccess, loginFailure);
    thunderrtc.setPeerListener(messageListener);
    thunderrtc.setDisconnectListener( function() {
        thunderrtc.showError("LOST-CONNECTION", "Lost connection to signaling server");
    });
    thunderrtc.setOnCall( function(thunderrtcid, slot) {
        boxUsed[slot+1] = true;
        if(activeBox === 0 &&  thunderrtc.getConnectionCount() === 1) { // first connection
            collapseToThumb();
            document.getElementById('textEntryButton').style.display = 'block';
        }
    ...
    thunderrtc.setOnHangup(function(thunderrtcid, slot) {
        ...
        console.log("hanging up on " + thunderrtcid);
        ...
        setTimeout(function() {
            ...
            if( thunderrtc.getConnectionCount() === 0 ) { // no more connections
                expandThumb(0);
                document.getElementById('textEntryButton').style.display = 'none';
                document.getElementById('textentryBox').style.display = 'none';
            }
