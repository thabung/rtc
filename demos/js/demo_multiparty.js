...
thunderrtc.dontAddCloseButtons(true);
...
function callEverybodyElse(roomName, otherPeople) {
    ...
    thunderrtc.setRoomOccupantListener(null); // so we're only called once.

    var list = [];
    var connectCount = 0;
    for(var thunderrtcid in otherPeople ) {
        list.push(thunderrtcid);
    }
    //
    ...
    if( stringToSend && stringToSend != "") {
        for(var i = 0; i < maxCALLERS; i++ ) {
            ...
            if( thunderrtcid && thunderrtcid != "") {
                thunderrtc.sendPeerMessage(thunderrtcid, "im",  stringToSend);
            }
        }
    }
    return false;
...
function appInit() {
    ...
    thunderrtc.easyApp("thunderrtc.multiparty", "box0", ["box1", "box2", "box3"], loginSuccess);
    ...
    thunderrtc.setOnCall( function(thunderrtcid, slot) {
        console.log("getConnection count="  + thunderrtc.getConnectionCount() );
        boxUsed[slot+1] = true;
