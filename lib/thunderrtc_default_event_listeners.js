/**
 * Event listeners used by ThunderRTC. Many of these can be overridden using server options.
 * 
 * @module      thunderrtc_default_event_listeners
...
/**
 * Event listeners used by ThunderRTC. Many of these can be overridden using server options. The interfaces should be used as a guide for creating new listeners.
 *
 * @class 
 */
 ...
 * @param       {Object} thunderrtcAuthMessage Message object containing the complete authentication message sent by the connection.
 * @param       {nextCallback} next     A success callback of form next(err).
 */
eventListener.onAuthenticate = function(socket, thunderrtcid, appName, username, credential, thunderrtcAuthMessage, next) {
    next(null);
};
 ...
 * 
 * @param       {Object} connectionObj  ThunderRTC connection object. Contains methods used for identifying and managing a connection.
 * @param       {nextCallback} next     A success callback of form next(err).
 */
 ...
 * @param       {nextCallback} next     A success callback of form next(err).
 */
eventListener.onConnection = function(socket, thunderrtcid, next){
    var connectionObj = {};             // prepare variables to house the connection object

    // Initially upon a connection, we are only concerned with receiving an thunderrtcAuth message
    socket.on("thunderrtcAuth", function(msg, socketCallback) {
        pub.events.emit("thunderrtcAuth", socket, thunderrtcid, msg, socketCallback, function(err, newConnectionObj){
            if(err){
                pub.util.logError("["+thunderrtcid+"] Unhandled thunderrtcCmd listener error.", err);
                return;
            }
...
eventListener.onDisconnect = function(connectionObj, next){
    ...
    async.waterfall([
        ...
        function(asyncCallback) {
            ...
            pub.util.logDebug("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] Disconnected");
...
/**
 * Default listener for event "thunderrtcAuth". This event is fired when an incoming 'thunderrtcAuth' message is received from a client.
 *
 * @param       {Object}    socket         Socket.io socket object. References the individual connection socket.
 * @param       {String}    thunderrtcid      Unique identifier for an ThunderRTC connection.
 ...
 */
eventListener.onThunderrtcAuth = function(socket, thunderrtcid, msg, socketCallback, callback){
    pub.util.logDebug("["+thunderrtcid+"] Running func 'onThunderrtcAuth'");
    ...
    if(!_.isFunction(socketCallback)) {
        pub.util.logWarning("["+thunderrtcid+"] ThunderRTC Auth message received with no callback. Disconnecting socket.", msg);
    ...
    if(!_.isObject(msg) || !_.isString(msg.msgType) || msg.msgType != "authenticate") {
        pub.util.logWarning("["+thunderrtcid+"] ThunderRTC Auth message received without msgType of 'authenticate'. Disconnecting socket.", msg);
    ...
    if(!_.isObject(msg.msgData)
        ...
        pub.util.logWarning("["+thunderrtcid+"] ThunderRTC Auth message received with improper msgData. Disconnecting socket.", msg);
    ...
    async.waterfall([
        function(asyncCallback) {
            ...
            pub.util.isValidIncomingMessage("thunderrtcAuth", msg, null, asyncCallback);
        },
        ...
        function(isMsgValid, msgErrorCode, asyncCallback) {
            ...
            if (!isMsgValid) {
                ...
                pub.util.logWarning("["+thunderrtcid+"] ThunderRTC Auth message received with invalid message format [" + msgErrorCode + "]. Disconnecting socket.", msg);
                callback(new pub.util.ConnectionError("["+thunderrtcid+"] ThunderRTC Auth message received with invalid message format [" + msgErrorCode + "]. Disconnecting socket."));
                return;
            }

            // Remove any old listeners
            socket.removeAllListeners("thunderrtcCmd");
            socket.removeAllListeners("thunderrtcMsg");
            ...
            pub.events.emit("authenticate", socket, thunderrtcid, newAppName, username, credential, msg, function(err){
                if (err) {
                    try{
                        socketCallback(pub.util.getErrorMsg("LOGIN_BAD_AUTH"));
                        socket.disconnect();
                        pub.util.logInfo("["+newAppName+"]["+thunderrtcid+"] Authentication denied. Socket disconnected.", err);
                    }catch(e){}
                } else {
                    asyncCallback(null);
                }
        ...
        function(isApp, asyncCallback) {
            ...
            if (isApp) {
                ...
                if(pub.getOption("appAutoCreateEnable")) {
                    ...
                    socket.disconnect();
                    pub.util.logWarning("[" + thunderrtcid + "] Authentication failed. Requested application not found [" + newAppName + "]. Socket disconnected.");
                }
            }
        },
        ...
        function(newAppObj, asyncCallback) {
            ...
            appObj.isConnected(thunderrtcid, asyncCallback);
        },
        ...
        function(isConnected, asyncCallback) {
            ...
            if (isConnected){
                socketCallback(pub.util.getErrorMsg("LOGIN_APP_AUTH_FAIL"));
                socket.disconnect();
                pub.util.logWarning("[" + thunderrtcid + "] Authentication failed. Already connected. Socket disconnected.");
                return;
            }
            ...
            if (msg.msgData.roomJoin) {
                for (var currentRoomName in msg.msgData.roomJoin) {
                    if (!_.isString(currentRoomName) || !appObj.getOption("roomNameRegExp").test(currentRoomName)) {
                        ...
                        pub.util.logInfo("[" + thunderrtcid + "] Authentication failed. Requested room name not allowed [" + currentRoomName + "].");
                        return;
                    }
                }
            }
        ...
        function(asyncCallback) {
            ...
            appObj.createConnection(thunderrtcid, asyncCallback);
        },
        ...
        function(newConnectionObj, asyncCallback) {
            ...
            // Check if there is an thunderrtcsid
            if (_.isString(msg.msgData.thunderrtcsid)) {
                appObj.isSession(msg.msgData.thunderrtcsid, function(err, isSession){
                    if (err) {
                        asyncCallback(err);
                        return;
                    }
                    if (isSession) {
                        appObj.session(msg.msgData.thunderrtcsid, asyncCallback);
                    } else {
                        appObj.createSession(msg.msgData.thunderrtcsid, asyncCallback);
                    }
                });
            }
            else {
        ...
        function(newSessionObj, asyncCallback) {
            ...
            connectionObj.joinSession(sessionObj.getThunderrtcsid(), asyncCallback);
        },
                ...
                async.each(Object.keys(msg.msgData.roomJoin), function(currentRoomName, roomCallback) {
                    ...
                    appObj.isRoom(currentRoomName, function(err, isRoom){
                        ...
                        if (isRoom) {
                            ...
                            pub.util.logInfo("[" + thunderrtcid + "] Authentication failed. Requested room name does not exist [" + currentRoomName + "].");
                        }
                    });
        ...
        function(asyncCallback) {
            // Add new listeners
            socket.on("thunderrtcCmd", function(msg, socketCallback){
                pub.events.emit("thunderrtcCmd", connectionObj, msg, socketCallback, function(err){
                    if(err){pub.util.logError("["+thunderrtcid+"] Unhandled thunderrtcCmd listener error.", err);}
                });

            });
            socket.on("thunderrtcMsg", function(msg, socketCallback){
                pub.events.emit("thunderrtcMsg", connectionObj, msg, socketCallback, function(err){
                    if(err){pub.util.logError("["+thunderrtcid+"] Unhandled thunderrtcMsg listener error.", err);}
                });
            });
            socket.on("disconnect", function(){
                pub.events.emit("disconnect", connectionObj, function(err){
                    if(err){pub.util.logError("["+thunderrtcid+"] Unhandled disconnect listener error.", err);}
                });
            });
            asyncCallback(null);
        },
    ...
    function (err) {
        if (err){
            try{
                ...
                socket.disconnect();
                pub.util.logError("["+thunderrtcid+"] General authentication error. Socket disconnected.", err);
            }catch(e){}
        } else {
...
/**
 * Default listener for event "thunderrtcCmd". This event is fired when an incoming 'thunderrtcCmd' message is received from a client.
 * 
 * @param       {Object} connectionObj  ThunderRTC connection object. Contains methods used for identifying and managing a connection.
 * @param       {Object} msg            Message object which contains the full message from a client; this can include the standard msgType and msgData fields.
 * @param       {Function} socketCallback Socket.io callback function which delivers a response to a socket. Expects a single parameter (msg).
 * @param       {nextCallback} next     A success callback of form next(err).
 */
eventListener.onThunderrtcCmd = function(connectionObj, msg, socketCallback, next){
    pub.util.logDebug("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] ThunderRTC command received with msgType [" + msg.msgType + "]");
    if (!_.isFunction(next)) {
        next = pub.util.nextToNowhere;
    }

    if(!_.isFunction(socketCallback)) {
        pub.util.logWarning("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] ThunderRTC command message received with no callback. Ignoring.", msg);
        return;
    }
    ...
    async.waterfall([
        function(asyncCallback) {
            // Check message structure
            pub.util.isValidIncomingMessage("thunderrtcCmd", msg, connectionObj.getApp(), asyncCallback);
        },
        ...
        function(isMsgValid, msgErrorCode, asyncCallback) {
            ...
            if (!isMsgValid) {
                ...
                pub.util.logWarning("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] ThunderRTC Auth message received with invalid message format [" + msgErrorCode + "]. Disconnecting socket.", msg);
                return;
            }
            asyncCallback(null);
        },
        function(asyncCallback) {
            ...
            switch(msg.msgType) {
                case "setUserCfg":
                    pub.util.logDebug("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] WebRTC setUserCfg command received. This feature is not yet complete.");
                ...
                case "reject":
                case "hangup":
                    // Relay message to targetThunderrtcid
                    var outgoingMsg = {senderThunderrtcid: connectionObj.getThunderrtcid(), msgData:msg.msgData};

                    connectionObj.getApp().connection(msg.targetThunderrtcid, function(err,targetConnectionObj){
                        if (err){
                            socketCallback(pub.util.getErrorMsg("MSG_REJECT_TARGET_EASYRTCID"));
                            pub.util.logWarning("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] Could not send WebRTC signal to client [" + msg.targetThunderrtcid + "]. They may no longer be online.");
                            return;
                        }
                        pub.events.emit("emitThunderrtcCmd", targetConnectionObj, msg.msgType, outgoingMsg, null, next);
                        socketCallback({msgType:'ack'});
                        next(null);
                    });
                    break;
                ...
                default:
                    ...
                    pub.util.logWarning("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] Received thunderrtcCmd message with unhandled msgType.", msg);
                    next(null);
            }
        }
    ],
    function(err) {
        if (err) {
            try {
                ...
                pub.util.logWarning("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] Received thunderrtcCmd message with general error.", msg);
            } catch(e){}
        }
    });
};
...
/**
 * Default listener for event "thunderrtcMsg". This event is fired when an incoming 'thunderrtcMsg' message is received from a client.
 * 
 * @param       {Object} connectionObj  ThunderRTC connection object. Contains methods used for identifying and managing a connection.
 * @param       {Object} msg            Message object which contains the full message from a client; this can include the standard msgType and msgData fields.
 * @param       {Function} socketCallback Socket.io callback function which delivers a response to a socket. Expects a single parameter (msg).
 * @param       {nextCallback} next     A success callback of form next(err).
 */
eventListener.onThunderrtcMsg = function(connectionObj, msg, socketCallback, next){
    pub.util.logDebug("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] ThunderRTC message received of type [" + msg.msgType + "]");
    ...
    if(!_.isFunction(socketCallback)) {
        pub.util.logWarning("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] ThunderRTC message received with no callback. Ignoring message.", msg);
        return;
    }
    ...
    async.waterfall([
        function(asyncCallback) {
            // Check message structure
            pub.util.isValidIncomingMessage("thunderrtcMsg", msg, connectionObj.getApp(), asyncCallback);
        },
        ...
        function(isMsgValid, msgErrorCode, asyncCallback) {
            ...
            if (!isMsgValid) {
                socketCallback(pub.util.getErrorMsg(msgErrorCode));
                pub.util.logWarning("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] ThunderRTC message received with invalid message format [" + msgErrorCode + "].", msg);
                return;
            }
            asyncCallback(null);
        },
        function(asyncCallback) {
            ...
            // test targetThunderrtcid (if defined). Will prevent client from sending to themselves
            if (msg.targetThunderrtcid  !== undefined && msg.targetThunderrtcid == connectionObj.getThunderrtcid()) {
                socketCallback(pub.util.getErrorMsg("MSG_REJECT_TARGET_EASYRTCID"));
                pub.util.logWarning("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] ThunderRTC message received with improper targetThunderrtcid", msg);
                return;
            }

            // Determine if sending message to single client, an entire room, or an entire group
            if (msg.targetThunderrtcid !== undefined) {
                // Relay a message to a single client
                var outgoingMsg = {
                    senderThunderrtcid: connectionObj.getThunderrtcid(),
                    targetThunderrtcid: msg.targetThunderrtcid,
                    msgType: msg.msgType,
                    msgData: msg.msgData
                };
                ...
                async.waterfall([
                    function(asyncCallback) {
                        // getting connection object for targetThunderrtcid
                        connectionObj.getApp().connection(msg.targetThunderrtcid, asyncCallback);
                    },
                        ...
                        if (msg.targetRoom) {
                            targetConnectionObj.isInRoom(msg.targetRoom, function(err, isAllowed){
                                if (err || !isAllowed) {
                                    ...
                                    pub.util.logWarning("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] ThunderRTC message received with improper target room", msg);
                                    return;
                                }
                        ...
                        if (msg.targetGroup) {
                            targetConnectionObj.isInGroup(msg.targetGroup, function(err, isAllowed){
                                if (err || !isAllowed) {
                                    ...
                                    pub.util.logWarning("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] ThunderRTC message received with improper target group", msg);
                                    return;
                                }
                    ...
                    function(asyncCallback) {
                        pub.events.emit("emitThunderrtcMsg", targetConnectionObj, msg.msgType, outgoingMsg, null, asyncCallback);
                    }
                ...
                function (err) {
                    if (err) {
                        socketCallback(pub.util.getErrorMsg("MSG_REJECT_GEN_FAIL"));
                        pub.util.logError("["+connectionObj.getThunderrtcid()+"] General message error. Message ignored.", err);
                    } else {
                        socketCallback({msgType:'ack'});
                    }
                });
                ...
                async.waterfall([
                    ...
                    function(connectedThunderrtcidArray, asyncCallback) {
                        for (var i = 0; i < connectedThunderrtcidArray.length; i++) {
                            // Stop client from sending message to themselves
                            if (connectedThunderrtcidArray[i] == connectionObj.getThunderrtcid()) {
                                continue;
                            }

                            connectionObj.getApp().connection(connectedThunderrtcidArray[i], function(err, targetConnectionObj){
                                if (err) {
                                    return;
                                }
                                ...
                                if(msg.targetGroup) {
                                    targetConnectionObj.isInGroup(msg.targetGroup, function(err, isAllowed){
                                        if (isAllowed) {
                                            pub.events.emit("emitThunderrtcMsg", targetConnectionObj, msg.msgType, outgoingMsg, null, pub.util.nextToNowhere);
                                        }
                                    });
                                }
                                else {
                ...
                pub.util.logWarning("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] ThunderRTC message received without targetThunderrtcid or targetRoom", msg);
                next(null);
            }
        }
    ],
...
/**
 * Default listener for event "emitThunderrtcCmd". This event is fired when the server should emit an ThunderRTC command to a client.
 * 
 * The thunderrtcid and serverTime fields will be added to the msg automatically.
 * 
 * @param       {Object} connectionObj  ThunderRTC connection object. Contains methods used for identifying and managing a connection.
 ...
 * @param       {nextCallback} next     A success callback of form next(err).
 */
eventListener.onEmitThunderrtcCmd = function(connectionObj, msgType, msg, socketCallback, next){
    if (!_.isObject(connectionObj)){
        next(new pub.util.ConnectionError("Connection object invalid. Client may have disconnected."));
        return;
    }

    pub.util.logDebug("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] Running func 'onEmitThunderrtcCmd' with msgType ["+msgType+"]");
    if (!msg) {
        msg = {};
    }
    if(!_.isFunction(socketCallback)) {
        socketCallback = function(returnMsg) {
            if (_.isObject(returnMsg) && _.isString(returnMsg.msgType) && returnMsg.msgType == "ack"){
                // pub.util.logDebug("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] ThunderRTC message: unhandled Ack return message.");
            }
            else {
                pub.util.logDebug("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] ThunderRTC message: unhandled return message.", returnMsg);
            }
        };
    }

    msg.thunderrtcid   = connectionObj.getThunderrtcid();
    msg.msgType     = msgType;
    msg.serverTime  = Date.now();

    connectionObj.socket.emit( "thunderrtcCmd", msg, socketCallback);
    next(null);
};


/**
 * Default listener for event "emitThunderrtcMsg". This event is fired when the server should emit an ThunderRTC message to a client.
 * 
 * The thunderrtcid and serverTime fields will be added to the msg automatically.
 * 
 * @param       {Object} connectionObj  ThunderRTC connection object. Contains methods used for identifying and managing a connection.
 ...
 * @param       {nextCallback} next     A success callback of form next(err).
 */
eventListener.onEmitThunderrtcMsg = function(connectionObj, msgType, msg, socketCallback, next){
    pub.util.logDebug("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] Running func 'onEmitThunderrtcMsg' with msgType ["+msgType+"]");
    ...
    connectionObj.socket.emit( "thunderrtcMsg", msg, socketCallback);
...
/**
 * Default listener for event "emitError". This event is fired when the server should emit an ThunderRTC error to a client.
 * 
 * @param       {Object} connectionObj  ThunderRTC connection object. Contains methods used for identifying and managing a connection.
...
eventListener.onEmitError = function(connectionObj, errorCode, socketCallback, next){
    pub.util.logDebug("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] Running func 'onEmitError'");
    if(!_.isFunction(socketCallback)) {
        socketCallback = function(returnMsg) {
            pub.util.logDebug("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] ThunderRTC info: unhandled ACK return message.", returnMsg);
        };
    }
    if(!_.isFunction(next)) {
        next = pub.util.nextToNowhere;
    ...
    pub.events.emit("emitThunderrtcCmd", connectionObj, "error", pub.util.getErrorMsg(errorCode), socketCallback, next);
};
...
eventListener.onEmitReturnAck = function(connectionObj, socketCallback, next){
    pub.util.logDebug("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] Running func 'onEmitReturnAck'");
    if(!_.isFunction(socketCallback)) {
        pub.util.logWarning("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] ThunderRTC: unable to return ack to socket.");
        return;
    }
    if(!_.isFunction(next)) {
        next = pub.util.nextToNowhere;
...
eventListener.onEmitReturnError = function(connectionObj, socketCallback, errorCode, next){
    pub.util.logDebug("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] Running func 'onEmitReturnError'");
    if(!_.isFunction(socketCallback)) {
        pub.util.logWarning("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] ThunderRTC: unable to return error to socket. Error code was [" + errorCode + "]");

        next(new pub.util.ConnectionError("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] Unable to return error to socket. Error code was [" + errorCode + "]"));
        return;
    }
    if(!_.isFunction(next)) {
        next = pub.util.nextToNowhere;
...
eventListener.onEmitReturnToken = function(connectionObj, socketCallback, next){
    pub.util.logDebug("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] Running func 'onSendToken'");
    ...
    // Ensure socketCallback is present
    if(!_.isFunction(socketCallback)) {
        pub.util.logWarning("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] ThunderRTC onSendToken called with no socketCallback.");
    ...
    async.waterfall([
        ...
        function(iceServers, asyncCallback) {
            ...
            tokenMsg.msgData.thunderrtcid          = connectionObj.getThunderrtcid();
        ...
        function(sessionObj, asyncCallback) {
            if (sessionObj) {
                tokenMsg.msgData.sessionData = {"thunderrtcsid":sessionObj.getThunderrtcsid()};
...
eventListener.onLog = function(level, logText, logFields, next) {
    ...
    if (pub.getOption("logColorEnable")) {
        ...
        consoleText += " - " + "ThunderRTC: ".bold + logText;
    }
    else {
        if(pub.getOption("logDateEnable")) {
        ...
        consoleText += level;
        consoleText += " - " + "ThunderRTC: " + logText;
    }
...
/**
 * Default listener for event "msgTypeRoomJoin". This event is fired when an thunderrtcCmd message with msgType of "roomJoin" is received from a client. 
 * 
 * @param       {Object} connectionObj  ThunderRTC connection object. Contains methods used for identifying and managing a connection.
...
eventListener.onMsgTypeRoomJoin = function(connectionObj, rooms, socketCallback, next){
    pub.util.logDebug("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] Running func 'onMsgTypeRoomJoin'");
    if(!_.isFunction(socketCallback)) {
        pub.util.logWarning("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] ThunderRTC info: unhandled socket message callback.");
        return;
    }
    ...
    async.each(Object.keys(rooms), function(currentRoomName, roomCallback) {
        appObj.isRoom(currentRoomName, function(err, isRoom){
            ...
            if (isRoom) {
                ...
                pub.util.logWarning("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"]["+currentRoomName+"] Unable to join non-existent room.");
                roomCallback(new pub.util.ConnectionError("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"]["+currentRoomName+"] Unable to join room."));
            }
        });
    }, function(err, newRoomObj) {
        if (err) {
...
/**
 * Default listener for event "msgTypeRoomLeave". This event is fired when an thunderrtcCmd message with msgType of "roomLeave" is received from a client. 
 * 
 * @param       {Object} connectionObj  ThunderRTC connection object. Contains methods used for identifying and managing a connection.
...
eventListener.onMsgTypeRoomLeave = function(connectionObj, rooms, socketCallback, next){
    pub.util.logDebug("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] Running func 'onMsgTypeRoomLeave' with rooms: ",rooms);
    if(!_.isFunction(socketCallback)) {
        socketCallback = function(returnMsg) {
            pub.util.logDebug("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] ThunderRTC info: unhandled ACK return message.", returnMsg);
        };
    ...
    async.each(Object.keys(rooms), function(currentRoomName, asyncCallback) {
        connectionObj.events.emit("roomLeave", connectionObj, currentRoomName, function(err){
            if (err) {
                pub.util.logWarning("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] Error leaving room ["+currentRoomName+"].", err);
            }
            asyncCallback(null);
        });
    }, function(err, newRoomObj) {
...
/**
 * Default listener for event "msgTypeGetIceConfig". This event is fired when an thunderrtcCmd message with msgType of "getIceConfig" is received from a client. 
 * 
 * @param       {Object} connectionObj  ThunderRTC connection object. Contains methods used for identifying and managing a connection.
 * @param       {Function} socketCallback Socket.io callback function which delivers a response to a socket. Expects a single parameter (msg).
 * @param       {nextCallback} next     A success callback of form next(err).
...
eventListener.onMsgTypeGetIceConfig = function(connectionObj, socketCallback, next){
    pub.util.logDebug("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] Running func 'onMsgTypeGetIceConfig'");
...
/**
 * Default listener for event "msgTypeGetRoomList". This event is fired when an thunderrtcCmd message with msgType of "getRoomList" is received from a client. 
 * 
 * @param       {Object} connectionObj  ThunderRTC connection object. Contains methods used for identifying and managing a connection.
 * @param       {Function} socketCallback Socket.io callback function which delivers a response to a socket. Expects a single parameter (msg).
 * @param       {nextCallback} next     A success callback of form next(err).
...
eventListener.onMsgTypeGetRoomList = function(connectionObj, socketCallback, next){
    pub.util.logDebug("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] Running func 'onMsgTypeGetRoomList'");
...
/**
 * Default listener for event "msgTypeSetPresence". This event is fired when an thunderrtcCmd message with msgType of "setPresence" is received from a client. 
 * 
 * @param       {Object} connectionObj  ThunderRTC connection object. Contains methods used for identifying and managing a connection.
...
eventListener.onMsgTypeSetPresence = function(connectionObj, presenceObj, socketCallback, next){
    pub.util.logDebug("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] Running func 'onMsgTypeSetPresence' with setPresence: ",presenceObj);
    if(!_.isFunction(socketCallback)) {
        socketCallback = function(returnMsg) {
            pub.util.logDebug("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] ThunderRTC info: unhandled ACK return message.", returnMsg);
        };
...
/**
 * Default listener for event "msgTypeSetRoomApiField". This event is fired when an thunderrtcCmd message with msgType of "setRoomApiField" is received from a client. 
 * 
 * @param       {Object} connectionObj  ThunderRTC connection object. Contains methods used for identifying and managing a connection.
...
eventListener.onMsgTypeSetRoomApiField = function(connectionObj, roomApiFieldObj, socketCallback, next){
    pub.util.logDebug("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] Running func 'onMsgTypeSetRoomApiField' with apiFieldObj: ",roomApiFieldObj);
    if(!_.isFunction(socketCallback)) {
        socketCallback = function(returnMsg) {
            pub.util.logDebug("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] ThunderRTC info: unhandled ACK return message.", returnMsg);
        };
...
eventListener.onGetIceConfig = function(connectionObj, callback){
    pub.util.logDebug("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] Running func 'onGetIceConfig'");
 ...
 * @param       {?Object} creatorConnectionObj ThunderRTC connection object belonging to the creator of the room. Contains methods used for identifying and managing a connection.
...
eventListener.onRoomCreate = function(appObj, creatorConnectionObj, roomName, roomOptions, callback){
    pub.util.logDebug("["+appObj.getAppName()+"]" + (creatorConnectionObj?"["+creatorConnectionObj.getThunderrtcid()+"]":"") +  " Running func 'onRoomCreate'");
 ...
 * @param       {Object} connectionObj  ThunderRTC connection object. Contains methods used for identifying and managing a connection.
 * @param       {string} roomName       Room name which uniquely identifies a room within an ThunderRTC application.
 * @param       {?Object} roomParameter A map(dictionary) object with key/value pairs. The values can be any JSONable object. This field is not currently looked at by ThunderRTC, however it is available for custom server applications. May be used for room options or authentication needs.
...
eventListener.onRoomJoin = function(connectionObj, roomName, roomParameter, callback){
    pub.util.logDebug("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] Running func 'onRoomJoin'");
    ...
    if (_.isFunction(roomParameter)){
        pub.util.logWarning("Upgrade notice: ThunderRTC roomJoin event called without roomParameter object.");
...
eventListener.onRoomLeave = function(connectionObj, roomName, next){
    pub.util.logDebug("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] Running func 'onRoomLeave' with rooms ["+roomName+"]");
    ...
    connectionObj.room(roomName, function(err, connectionRoomObj){
        if (err) {
            pub.util.logWarning("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] Couldn't leave room [" + roomName + "]");
            next(err);
            return;
        }

        pub.util.logDebug("["+connectionObj.getAppName()+"]["+connectionObj.getThunderrtcid()+"] Leave room [" + roomName + "]");
...
/**
 * Default listener for event "startup". This event initializes ThunderRTC server so it is ready for connections.
 *
 * @param       {nextCallback} next     A success callback of form next(err).
 */
    ...
    async.waterfall([
        function(callback) {
            ...
            if (pub.getOption("apiEnable")) {
                // Set the ThunderRTC API files
                pub.util.logDebug("Setting up API files to be accessed from '" + pub.getOption("apiPublicFolder") + "/'");
                pub.httpApp.get(pub.getOption("apiPublicFolder") + "/thunderrtc.js",                  function(req, res) {pub.util.sendSessionCookie(req, res); (res.sendFile||res.sendfile).call(res,"api/thunderrtc.js",                   {root:__dirname + "/../"});});
                pub.httpApp.get(pub.getOption("apiPublicFolder") + "/thunderrtc_ft.js",               function(req, res) {pub.util.sendSessionCookie(req, res); (res.sendFile||res.sendfile).call(res,"api/thunderrtc_ft.js",                {root:__dirname + "/../"});});
                pub.httpApp.get(pub.getOption("apiPublicFolder") + "/thunderrtc.css",                 function(req, res) {pub.util.sendSessionCookie(req, res); (res.sendFile||res.sendfile).call(res,"api/thunderrtc.css",                  {root:__dirname + "/../"});});
                pub.httpApp.get(pub.getOption("apiPublicFolder") + "/thunderrtc.min.js",              function(req, res) {pub.util.sendSessionCookie(req, res); (res.sendFile||res.sendfile).call(res,"open_source/api/thunderrtc.min.js",   {root:__dirname + "/../"});});
                pub.httpApp.get(pub.getOption("apiPublicFolder") + "/thunderrtc_ft.min.js",           function(req, res) {pub.util.sendSessionCookie(req, res); (res.sendFile||res.sendfile).call(res,"api/thunderrtc_ft.min.js",            {root:__dirname + "/../"});});
                pub.httpApp.get(pub.getOption("apiPublicFolder") + "/thunderrtc.min.css",             function(req, res) {pub.util.sendSessionCookie(req, res); (res.sendFile||res.sendfile).call(res,"open_source/api/thunderrtc.min.css",  {root:__dirname + "/../"});});
            ...
            if (pub.getOption("apiEnable") && pub.getOption("apiOldLocationEnable")) {
                ...
                // Transition - Old locations of ThunderRTC API files
                pub.httpApp.get("/js/thunderrtc.js",                   function(req, res) {(res.sendFile||res.sendfile).call(res,"api/thunderrtc.js",              {root:__dirname + "/../"});});
                pub.httpApp.get("/css/thunderrtc.css",                 function(req, res) {(res.sendFile||res.sendfile).call(res,"api/thunderrtc.css",             {root:__dirname + "/../"});});
            }
            callback(null);
        },
        ...
        function(callback) {
            ...
            pub.socketServer.sockets.on("connection", function (socket) {
                var thunderrtcid = socket.id;

                pub.util.logDebug("["+thunderrtcid+"] Socket connected");
                pub.util.logDebug("Emitting event 'connection'");
                pub.events.emit("connection", socket, thunderrtcid, function(err){
                    if(err){
                        socket.disconnect();
                        pub.util.logError("["+thunderrtcid+"] Connect error", err);
                    }
                });
            });
            callback(null);
        ...
        function(appObj, callback) {
            // Checks to see if there is a newer version of ThunderRTC available
