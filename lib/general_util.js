/**
 * @file        General utility functions not specific to ThunderRTC
 * @module      general_util
...
g.checkModules = function () {
    try {
        var thunderrtcPackage = require("../package");
    }
    catch( e ) {
    ...
    for (var key in thunderrtcPackage.dependencies) {
        if (!moduleExists(key)) {
    ...
    delete require.cache[thunderrtcPackage];
};
