/**
 * @file        Default options used within ThunderRTC. Overriding of default options should be done using the public listen() or setOption() functions.
 * @module      thunderrtc_default_options
...
option.sessionCookieEnable  = true;                         // If enabled, the server will attempt to send a thunderrtcsid cookie which matches the Express session id.


// API Hosting Options
option.apiEnable            = true;                         // Enables hosting of the ThunderRTC API files.
option.apiPublicFolder      = "/thunderrtc";                   // Api public folder without trailing slash. Note that the demos expect this to be '/thunderrtc'
option.apiLabsEnable        = true;                         // Enables hosting of the ThunderRTC experimental API files located in the 'labs' sub folder
...
option.thunderrtcidRegExp      = /^[a-z0-9_.-]{1,32}$/i;       // ThunderRTC socket id (thunderrtcid)
option.thunderrtcsidRegExp     = /^[a-z0-9_.-]{1,64}$/i;       // ThunderRTC session id (thunderrtcsid)
