/**
 * Public interface for interacting with ThunderRTC. Contains the public object returned by the ThunderRTC listen() function.
 *
 * @module      thunderrtc_public_obj
...
var e = require("./thunderrtc_private_obj");     // ThunderRTC private object
var eventListener = require("./thunderrtc_default_event_listeners"); // ThunderRTC default event listeners
var eu = require("./thunderrtc_util");            // ThunderRTC utility functions

/**
 * The public object which is returned by the ThunderRTC listen() function. Contains all public methods for interacting with ThunderRTC server.
 *
 * @class
 */
...
/**
 * Gets app object for application which has an authenticated client with a given thunderrtcid
 *
 * @param       {String} thunderrtcid      Unique identifier for an ThunderRTC connection.
 * @param       {function(?Error, Object=)} callback Callback with error and application object
 */
pub.getAppWithThunderrtcid = function(thunderrtcid, callback) {
    for (var key in e.app) {
        if (e.app[key].connection[thunderrtcid] && e.app[key].connection[thunderrtcid].isAuthenticated) {
            pub.app(key, callback);
            return;
        }
    }
    pub.util.logWarning("Can not find connection [" + thunderrtcid + "]");
    callback(new pub.util.ConnectionWarning("Can not find connection [" + thunderrtcid + "]"));
};
 ...
 *
 * @param       {function(?Error, Integer)} callback Callback with error and array containing all thunderrtcids.
 */
...
/**
 * Gets connection object for connection which has an authenticated client with a given thunderrtcid
 *
 * @param       {string} thunderrtcid      ThunderRTC unique identifier for a socket connection.
 * @param       {function(?Error, Object=)} callback Callback with error and connection object
 */
pub.getConnectionWithThunderrtcid = function(thunderrtcid, callback) {
    for (var key in e.app) {
        if (e.app[key].connection[thunderrtcid] && e.app[key].connection[thunderrtcid].isAuthenticated) {
            pub.app(key, function(err, appObj) {
                ...
                appObj.connection(thunderrtcid, callback);
            });
            return;
        }
    }
...
/**
 * Gets ThunderRTC Version. The format is in a major.minor.patch format with an optional letter following denoting alpha or beta status. The version is retrieved from the package.json file located in the ThunderRTC project root folder.
 *
 * @return      {string}                ThunderRTC Version
 */
...
/**
 * Returns the ThunderRTC private object containing the current state. This should only be used for debugging purposes.
 *
 * @private
 * @return      {Object}                ThunderRTC private object
 */
...
/**
 * ThunderRTC Event handling object which contain most methods for interacting with ThunderRTC events. For convenience, this class has also been attached to the application, connection, session, and room classes.
 * @class
 */
pub.events = {};
 ...
 * 
 * @param       {string} eventName      ThunderRTC event name.
...
/**
 * Runs the default ThunderRTC listener for a given event.
 * 
 * @param       {string} eventName      ThunderRTC event name.
 * @param       {...*} eventParam       The event parameters
 */
...
/**
 * Resets the listener for all ThunderRTC events to the default listeners. Removes all other listeners.
 */
...
/**
 * Map of ThunderRTC event listener names to their default functions. This map can be used to run a default function manually.
 */
pub.events.defaultListeners = {
    ...
    "thunderrtcAuth": eventListener.onThunderrtcAuth,
    "thunderrtcCmd": eventListener.onThunderrtcCmd,
    "thunderrtcMsg": eventListener.onThunderrtcMsg,
    "emitThunderrtcCmd": eventListener.onEmitThunderrtcCmd,
    "emitThunderrtcMsg": eventListener.onEmitThunderrtcMsg,
...
/**
 * Sets listener for a given ThunderRTC event. Only one listener is allowed per event. Any other listeners for an event are removed before adding the new one. See the events documentation for expected listener parameters.
 *
...
/**
 * Removes all listeners for an event. If there is a default ThunderRTC listener, it will be added. If eventName is `null`, all events will be removed than the defaults will be restored.
 *
...
/**
 * Custom Error Object for ThunderRTC Application Errors.
 *
 * @extends     Error
...
/**
 * Custom Error Object for ThunderRTC Application Warnings.
 *
 * @extends     Error
 * @param       {string} msg            Text message describing the error.
 * @returns     {Error}
...
/**
 * Custom Error Object for ThunderRTC C Errors.
 *
 * @function
 * @extends     Error
 * @param       {string} msg            Text message describing the error.
...
/**
 * Custom Error Object for ThunderRTC Connection Warnings.
 *
 * @function
 * @extends     Error
 * @param       {string} msg            Text message describing the error.
...
/**
 * Custom Error Object for ThunderRTC Server Errors.
 *
 * @function
 * @extends     Error
 * @param       {string} msg            Text message describing the error.
...
/**
 * Custom Error Object for ThunderRTC Server Warnings.
 *
 * @function
 * @extends     Error
 * @param       {string} msg            Text message describing the error.
...
/**
 * Returns an ThunderRTC message error object for a specific error code. This is meant to be emitted or returned to a websocket client.
 *
 * @param       {String} errorCode      ThunderRTC error code associated with an error.
 * @return      {Object}                ThunderRTC message error object for the specific error code.
 */
...
pub.util.getErrorText = function(errorCode) {
    switch (errorCode) {
        ...
        case "MSG_REJECT_TARGET_EASYRTCID":
            return "Message rejected. Target thunderrtcid is invalid, not using same application, or no longer online.";
            break;
...
/**
 *  Checks with ThunderRTC site for latest version. Writes to the log if a version can be found. If connection cannot be established than no error will be shown.
 */
pub.util.updateCheck = function() {
    var thunderrtcVersion = pub.getVersion();

    require("http").get("http://thunderrtc.com/version/?app=thunderrtc&ver=" + thunderrtcVersion + "&platform=" + process.platform + "&nodever=" + process.version, function(res) {
        if (res.statusCode == 200)
            res.on('data', function(latestVersion) {
                latestVersion = (latestVersion + "").replace(/[^0-9a-z.]/g, "");
                if (latestVersion != thunderrtcVersion) {
                    var l = latestVersion.replace(/[^0-9.]/g, "").split(".", 3);
                    l[0] = parseInt(l[0]);
                    l[1] = parseInt(l[1]);
                    l[2] = parseInt(l[2]);
                    var v = thunderrtcVersion.replace(/[^0-9.]/g, "").split(".", 3);
                    v[0] = parseInt(v[0]);
                    v[1] = parseInt(v[1]);
                    v[2] = parseInt(v[2]);
                    if (v[0] < l[0] || (v[0] == l[0] && v[1] < l[1]) || (v[0] == l[0] && v[1] == l[1] && v[2] < l[2]))
                        pub.util.logWarning("Update Check: New version of ThunderRTC is available (" + latestVersion + "). Visit http://thunderrtc.com/ for details or run 'npm update' to upgrade.");
                    else if (v[0] == l[0] && v[1] == l[1] && v[2] == l[2] && thunderrtcVersion.replace(/[^a-z]/gi, "") != "")
                        pub.util.logWarning("Update Check: New non-beta version of ThunderRTC is available (" + latestVersion + "). Visit http://thunderrtc.com/ for details.");
                }
            });
    }).on('error', function(e) {
    });
...
/**
 * Checks an incoming ThunderRTC message to determine if it is syntactically valid.
 *
 * @param       {string} type           The Socket.IO message type. Expected values are (thunderrtcAuth|thunderrtcCmd|thunderrtcMsg)
 ...
 * @param       {?Object} appObj        ThunderRTC application object. Contains methods used for identifying and managing an application.
...
pub.util.isValidIncomingMessage = function(type, msg, appObj, callback) {
    ...
    switch (type) {
        case "thunderrtcAuth":
            ...
            // msgData.thunderrtcsid
            if (msg.msgData.thunderrtcsid !== undefined && (!_.isString(msg.msgData.thunderrtcsid) || !getOption("thunderrtcsidRegExp").test(msg.msgData.thunderrtcsid))) {
                callback(null, false, "MSG_REJECT_BAD_STRUCTURE");
                return;
            }
        ...
        case "thunderrtcCmd":
            switch (msg.msgType) {
                ...
                case "offer" :
                case "answer" :
                    // candidate, offer, and answer each require a non-empty msgData object and a proper targetThunderrtcid
                    if (!_.isObject(msg.msgData) || _.isEmpty(msg.msgData)) {
                        callback(null, false, "MSG_REJECT_BAD_DATA");
                        return;
                    }
                    if (!_.isString(msg.targetThunderrtcid) || !getOption("thunderrtcidRegExp").test(msg.targetThunderrtcid)) {
                        callback(null, false, "MSG_REJECT_BAD_STRUCTURE");
                        return;
                    }
                    break;
                case "reject" :
                case "hangup" :
                    // reject, and hangup each require a targetThunderrtcid but no msgData
                ...
                case "setUserCfg" :
                    ...
                    // TODO: Go through p2pList to confirm each key is an thunderrtcid
        ...
        case "thunderrtcMsg":
            // targetThunderrtcid
            if (msg.targetThunderrtcid !== undefined && (!_.isString(msg.targetThunderrtcid) || !getOption("thunderrtcidRegExp").test(msg.targetThunderrtcid))) {
...
/**
 * Will attempt to deliver an ThunderRTC session id via a cookie. Requires that session management be enabled from within Express.
 *
...
pub.util.sendSessionCookie = function(req, res) {
    ...
    if (req.sessionID && (!req.cookies || !req.cookies["thunderrtcsid"] || req.cookies["thunderrtcsid"] != req.sessionID)) {
        try {
            pub.util.logDebug("Sending thunderrtcsid cookie [" + req.sessionID + "] to [" + req.ip + "] for request [" + req.url + "]");
            res.cookie("thunderrtcsid", req.sessionID, {maxAge: 2592000000, httpOnly: false});
        } catch (e) {
            pub.util.logWarning("Problem setting thunderrtcsid cookie [" + req.sessionID + "] to [" + req.ip + "] for request [" + req.url + "]");
        }
    }
};
...
/**
 * Creates a new ThunderRTC application with default values. If a callback is provided, it will receive the new application object.
 *
...
/**
 * Contains the methods for interfacing with an ThunderRTC application.
 *
...
pub.app = function(appName, callback) {
    ...
    /**
     * The primary method for interfacing with an ThunderRTC application.
     *
     * @class       appObj
     * @memberof    pub
     */
    ...
    /**
     * Returns the application name for the application. Note that unlike most ThunderRTC functions, this returns a value and does not use a callback.
     *
     * @memberof    pub.appObj
    ...
    /**
     * Returns an array of all thunderrtcids connected to the application
     *
     * @memberof    pub.appObj
     * @param       {function(?Error, Array.<string>)} callback Callback with error and array of thunderrtcids.
     */
    appObj.getConnectionThunderrtcids = function(callback) {
        var thunderrtcids = [];
        for (var key in e.app[appName].connection) {
            thunderrtcids.push(key);
        }
        callback(null, thunderrtcids);
    };
    ...
    /**
     * Returns an array of all thunderrtcsids within the application
     *
     * @memberof    pub.appObj
     * @param       {function(?Error, Array.<string>)} callback Callback with error and array containing thunderrtcsids.
     */
    appObj.getThunderrtcsids = function(callback) {
        var thunderrtcsids = [];
        for (var key in e.app[appName].session) {
            thunderrtcsids.push(key);
        }
        callback(null, thunderrtcsids);
    };

    /**
     * Returns an array of all thunderrtcsids within the application. Old SessionKey name kept for transition purposes. Use getThunderrtcsid();
     *
     * @memberof    pub.appObj
     * @ignore
     */
    appObj.getSessionKeys = appObj.getThunderrtcsids;
     ...
     * @memberof    pub.appObj
     * @param       {string}    thunderrtcid   ThunderRTC unique identifier for a socket connection.
     * @param       {function(?Error, Boolean)} callback Callback with error and a boolean indicating if thunderrtcid is connected.
     */
    appObj.isConnected = function(thunderrtcid, callback) {
        if (e.app[appName] && e.app[appName].connection && e.app[appName].connection[thunderrtcid]) {
            callback(null, true);
        } else {
            callback(null, false);
        }
    ...
    appObj.setOption = function(optionName, optionValue) {
        ...
        if (optionValue == null) {
            ...
            // Set the option value to be a full deep copy, thus preserving private nature of the private ThunderRTC object.
     ...
     * @memberof    pub.appObj
     * @param       {string}    thunderrtcid   ThunderRTC unique identifier for a socket connection.
     * @param       {connectionCallback} callback Callback with error and object containing ThunderRTC connection object.
     */
    appObj.connection = function(thunderrtcid, callback) {
        if (!e.app[appName].connection[thunderrtcid]) {
            pub.util.logWarning("Attempt to request non-existent connection key: '" + thunderrtcid + "'");
            callback(new pub.util.ConnectionWarning("Attempt to request non-existent connection key: '" + thunderrtcid + "'"));
            return;
        }

        if (!pub.socketServer) {
            pub.util.logError("Socket server undefined.");
            callback(new pub.util.ConnectionWarning("Attempt to request non-existent socket: '" + thunderrtcid + "'"));
            return;
        }

        if (pub.socketServer.sockets.connected){
            if (!pub.socketServer.sockets.connected[thunderrtcid] || pub.socketServer.sockets.connected[thunderrtcid].disconnected) {
                pub.util.logWarning("Attempt to request non-existent socket: '" + thunderrtcid + "'");
                callback(new pub.util.ConnectionWarning("Attempt to request non-existent socket: '" + thunderrtcid + "'"));
                return;
            }

            if (pub.socketServer.sockets.connected[thunderrtcid].disconnected) {
                pub.util.logWarning("Attempt to request disconnected socket: '" + thunderrtcid + "'");
                callback(new pub.util.ConnectionWarning("Attempt to request disconnected socket: '" + thunderrtcid + "'"));
                return;
            }
        }
        else {
            if (!pub.socketServer.sockets.sockets[thunderrtcid] || pub.socketServer.sockets.sockets[thunderrtcid].disconnected) {
                pub.util.logWarning("Attempt to request non-existent socket: '" + thunderrtcid + "'");
                callback(new pub.util.ConnectionWarning("Attempt to request non-existent socket: '" + thunderrtcid + "'"));
                return;
            }

            if (pub.socketServer.sockets.sockets[thunderrtcid].disconnected) {
                pub.util.logWarning("Attempt to request disconnected socket: '" + thunderrtcid + "'");
                callback(new pub.util.ConnectionWarning("Attempt to request disconnected socket: '" + thunderrtcid + "'"));
                return;
            }
         ...
         */
        if (pub.socketServer.sockets.connected){
            connectionObj.socket = pub.socketServer.sockets.connected[thunderrtcid];
        }
        else {
            connectionObj.socket = pub.socketServer.sockets.sockets[thunderrtcid];
        }


        /**
         * Returns the application object to which the connection belongs. Note that unlike most ThunderRTC functions, this returns a value and does not use a callback.
         *
         * @memberof    pub.appObj.connectionObj
        ...
        /**
         * Returns the application name for the application to which the connection belongs. Note that unlike most ThunderRTC functions, this returns a value and does not use a callback.
         *
         * @memberof    pub.appObj.connectionObj
         * @return      {string}    The application name
         */
        ...
        /**
         * Returns the thunderrtcid for the connection.  Note that unlike most ThunderRTC functions, this returns a value and does not use a callback.
         *
         * @memberof    pub.appObj.connectionObj
         * @return      {string}    Returns the connection's thunderrtcid, which is the ThunderRTC unique identifier for a socket connection.
         */
        connectionObj.getThunderrtcid = function() {
            return thunderrtcid;
        };
        ...
        connectionObj.getField = function(fieldName, callback) {
            if (!e.app[appName].connection[thunderrtcid].field[fieldName]) {
            ...
            callback(null, pub.util.deepCopy(e.app[appName].connection[thunderrtcid].field[fieldName]));
        };
        ...
        connectionObj.getFieldSync = function(fieldName) {
            ...
            return pub.util.deepCopy(e.app[appName].connection[thunderrtcid].field[fieldName]);
        };
        ...
        connectionObj.getFieldValueSync = function(fieldName) {
            ...
            return pub.util.deepCopy(e.app[appName].connection[thunderrtcid].field[fieldName].fieldValue);
        };
        ...
        connectionObj.getFields = function(limitToIsShared, callback) {
            var fieldObj = {};
            for (var fieldName in e.app[appName].connection[thunderrtcid].field) {
                if (!limitToIsShared || e.app[appName].connection[thunderrtcid].field[fieldName].fieldOption.isShared) {
                    fieldObj[fieldName] = {
                        fieldName: fieldName,
                        fieldValue: pub.util.deepCopy(e.app[appName].connection[thunderrtcid].field[fieldName].fieldValue)
                    };
                }
            }
            callback(null, fieldObj);
        ...
        connectionObj.getRoomNames = function(callback) {
            var roomNames = [];
            for (var key in e.app[appName].connection[thunderrtcid].room) {
                roomNames.push(key);
            }
            callback(null, roomNames);
        };
        ...
        /**
         * Returns the session object to which the connection belongs (if one exists). Returns a null if connection is not attached to a session (such as when sessions are disabled). Note that unlike most ThunderRTC functions, this returns a value and does not use a callback.
         *
         * @memberof    pub.appObj.connectionObj
        ...
        connectionObj.getSessionObj = function(callback) {
            if (e.app[appName].connection[thunderrtcid] && e.app[appName].connection[thunderrtcid].toSession && e.app[appName].connection[thunderrtcid].toSession.thunderrtcsid) {
                appObj.session(e.app[appName].connection[thunderrtcid].toSession.thunderrtcsid, callback);
            }
            else {
                callback(null, null);
            }
         ...
         * Note that unlike most ThunderRTC functions, this returns a value and does not use a callback.
         *
         * @memberof    pub.appObj.connectionObj
         * @return      {String}    The username associated with the connection.
         */
        connectionObj.getUsername = function() {
            return e.app[appName].connection[thunderrtcid].username;
        };
         ...
         *
         * @memberof    pub.appObj.connectionObj
         * @param       {string}    thunderrtcsid      ThunderRTC session identifier
         * @param       {nextCallback} next         A success callback of form next(err).
         */
        connectionObj.joinSession = function(thunderrtcsid, next) {
            if (!e.app[appName].session[thunderrtcsid]) {
                next(new pub.util.ConnectionWarning("[" + appName + "][" + thunderrtcid + "] Session [" + thunderrtcsid + "] does not exist. Could not join session"));
                return;
            }

            appObj.session(thunderrtcsid, function(err, sessionObj) {
                if (err) {
                    next(err);
                    return;
                }

                if(!e.app[appName] || !e.app[appName].connection[thunderrtcid] || !e.app[appName].session[thunderrtcsid]) {
                    next(new pub.util.ConnectionWarning("[" + appName + "][" + thunderrtcid + "] Session [" + thunderrtcsid + "] does not exist. Could not join session"));
                    return;
                }

                e.app[appName].connection[thunderrtcid].toSession = e.app[appName].session[thunderrtcsid];
                e.app[appName].connection[thunderrtcid].toSession.toConnection[thunderrtcid] = e.app[appName].connection[thunderrtcid];
        ...
        connectionObj.setAuthenticated = function(isAuthenticated, next) {
            if (isAuthenticated) {
                e.app[appName].connection[thunderrtcid].isAuthenticated = true;
            } else {
                e.app[appName].connection[thunderrtcid].isAuthenticated = false;
            }
            next(null);
        };
        ...
        connectionObj.setCredential = function(credential, next) {
            e.app[appName].connection[thunderrtcid].credential = credential;
            next(null);
        };
        ...
        connectionObj.setField = function(fieldName, fieldValue, fieldOption, next) {
            pub.util.logDebug("[" + appName + "][" + thunderrtcid + "] - Setting field [" + fieldName + "]", fieldValue);
            if (!_.isFunction(next)) {
                next = pub.util.nextToNowhere;
            }
            ...
            e.app[appName].connection[thunderrtcid].field[fieldName] = {
                fieldName: fieldName,
                fieldValue: fieldValue,
                fieldOption: {isShared: ((_.isObject(fieldOption) && fieldOption.isShared) ? true : false)}
            };
        ...
        connectionObj.setPresence = function(presenceObj, next) {
            if (presenceObj.show !== undefined) {
                e.app[appName].connection[thunderrtcid].presence.show = presenceObj.show;
            }
            if (presenceObj.status !== undefined) {
                e.app[appName].connection[thunderrtcid].presence.status = presenceObj.status;
            }
            if (presenceObj.type !== undefined) {
                e.app[appName].connection[thunderrtcid].presence.type = presenceObj.type;
            }
            next(null);
        };
        ...
        connectionObj.setUsername = function(username, next) {
            e.app[appName].connection[thunderrtcid].username = username;
            next(null);
        };
        ...
        connectionObj.emitRoomDataDelta = function(isLeavingAllRooms, callback) {
            pub.util.logDebug("[" + appName + "][" + thunderrtcid + "] Running func 'connectionObj.emitRoomDataDelta'");
            if (!_.isFunction(callback)) {
            ...
            connectionObj.generateRoomDataDelta(isLeavingAllRooms, function(err, newFullRoomDataDelta) {
                ...
                for (var currentRoomName in fullRoomDataDelta) {
                    for (var currentThunderrtcid in e.app[appName].room[currentRoomName].clientList) {
                        if (otherClients[currentThunderrtcid] === undefined) {
                            otherClients[currentThunderrtcid] = {};
                        }
                        otherClients[currentThunderrtcid][currentRoomName] = true;
                    }
                }

                // Emit custom roomData object to each client who shares a room with the current client
                for (var currentThunderrtcid in otherClients) {
                    var msg = {
                        "msgData": {
                            "roomData": {}
                        }
                    ...
                    for (var currentRoomName in otherClients[currentThunderrtcid]) {
                    ...
                    (function(innerCurrentThunderrtcid, innerMsg){
                        connectionObj.getApp().connection(innerCurrentThunderrtcid, function(err, emitToConnectionObj) {
                            if (!err && innerCurrentThunderrtcid != thunderrtcid && emitToConnectionObj) {
                                pub.events.emit("emitThunderrtcCmd", emitToConnectionObj, "roomData", innerMsg, null, function() {});
                            }
                        });
                    })(currentThunderrtcid, msg);
                }
            });
        };
        ...
        connectionObj.generateRoomClientList = function(roomStatus, roomMap, callback) {
            ...
            if (!_.isObject(roomMap)) {
                roomMap = e.app[appName].connection[thunderrtcid].room;
            }

            var roomData = {};

            for (var currentRoomName in e.app[appName].connection[thunderrtcid].room) {
                // If room is not in the provided roomMap, then skip it.
                if (!roomMap[currentRoomName]) {
                    continue;
                }

                var connectionRoom = e.app[appName].connection[thunderrtcid].room[currentRoomName];
                ...
                for (var currentThunderrtcid in connectionRoom.toRoom.clientList) {

                    var currentToConnection = connectionRoom.toRoom.clientList[currentThunderrtcid].toConnection;

                    connectionRoom.clientList[currentThunderrtcid] = {
                        "toConnection": currentToConnection
                    };

                    roomData[currentRoomName].clientList[currentThunderrtcid] = {
                        "thunderrtcid": currentThunderrtcid,
                    ...
                    if (currentToConnection.room[currentRoomName] && (!_.isEmpty(currentToConnection.room[currentRoomName].apiField))) {
                        roomData[currentRoomName].clientList[currentThunderrtcid].apiField = currentToConnection.room[currentRoomName].apiField;
                    }

                    if (currentToConnection.username) {
                        roomData[currentRoomName].clientList[currentThunderrtcid].username = currentToConnection.username;
                    }
                }
        ...
        connectionObj.generateRoomDataDelta = function(isLeavingRoom, callback) {
            pub.util.logDebug("[" + appName + "][" + thunderrtcid + "] Running func 'connectionObj.generateRoomDataDelta'");
            ...
            for (var currentRoomName in e.app[appName].connection[thunderrtcid].room) {
                ...
                if (isLeavingRoom) {
                    ...
                    roomDataDelta[currentRoomName].clientListDelta.removeClient[thunderrtcid] = {"thunderrtcid": thunderrtcid};
                } else {
                    roomDataDelta[currentRoomName].clientListDelta.updateClient = {};
                    roomDataDelta[currentRoomName].clientListDelta.updateClient[thunderrtcid] = {
                        "thunderrtcid": thunderrtcid,
                        "roomJoinTime": e.app[appName].connection[thunderrtcid].room[currentRoomName].enteredOn,
                        "presence": e.app[appName].connection[thunderrtcid].presence
                    };

                    if (!_.isEmpty(e.app[appName].connection[thunderrtcid].apiField)) {
                        roomDataDelta[currentRoomName].clientListDelta.updateClient[thunderrtcid].apiField = e.app[appName].connection[thunderrtcid].apiField;
                    }
                    if (e.app[appName].connection[thunderrtcid].username) {
                        roomDataDelta[currentRoomName].clientListDelta.updateClient[thunderrtcid].username = e.app[appName].connection[thunderrtcid].username;
                    }
                }
            }
        ...
        connectionObj.generateRoomList = function(callback) {
            pub.util.logDebug("[" + appName + "][" + thunderrtcid + "] Running func 'connectionObj.generateRoomList'");
            var roomList = {};
        ...
        /**
         * Gets connection authentication status for the connection. It is possible for a connection to become disconnected and keep the authenticated flag. Note that unlike most ThunderRTC functions, this returns a value and does not use a callback.
         *
         * @memberof    pub.appObj.connectionObj
         * @returns     {Boolean}   Authentication status
         */
        connectionObj.isAuthenticated = function() {
            if (e.app[appName].connection[thunderrtcid] && e.app[appName].connection[thunderrtcid].isAuthenticated) {
                return true;
            } else {
                return false;
            }
        ...
        /**
         * Gets connection status for the connection. It is possible for a connection to be considered connected without being authenticated. Note that unlike most ThunderRTC functions, this returns a value and does not use a callback.
         *
         * @memberof    pub.appObj.connectionObj
        ...
        connectionObj.isInGroup = function(groupName, callback) {
            if (_.isString(groupName) && e.app[appName].connection[thunderrtcid].group[groupName] !== undefined) {
                callback(null, true);
            }
            else {
                callback(null, false);
         ...
         *
         * @memberof    pub.appObj.connectionObj
         * @param       {string} roomName       Room name which uniquely identifies a room within an ThunderRTC application.
         * @param       {function(?Error, Boolean)} callback Callback with error and a boolean indicating if connection is in a room..
         */
        connectionObj.isInRoom = function(roomName, callback) {
            if (_.isString(roomName) && e.app[appName].connection[thunderrtcid].room[roomName] !== undefined) {
                callback(null, true);
            }
            else {
                callback(null, false);
         ...
         * @memberof    pub.appObj.connectionObj
         * @param       {string} roomName       Room name which uniquely identifies a room within an ThunderRTC application.
         * @param       {function(?Error, Object=)} callback Callback with error and object containing ThunderRTC connection room object (same as calling room(roomName))
         */
        connectionObj.joinRoom = function(roomName, callback) {
            if (!roomName || !appObj.getOption("roomNameRegExp").test(roomName)) {
                pub.util.logWarning("[" + appName + "][" + thunderrtcid + "] Can not enter room with improper name: '" + roomName + "'");
            ...
            if (!appObj.isRoomSync(roomName)) {
                pub.util.logWarning("[" + appName + "][" + thunderrtcid + "] Can not enter room which doesn't exist: '" + roomName + "'");
                callback(new pub.util.ConnectionWarning("Can not enter room which doesn't exist: '" + roomName + "'"));
                return;
            }

            // Check if client already in room
            if (e.app[appName].connection[thunderrtcid].room[roomName]) {
            ...
            var createConnectionRoom = function(roomName, appRoomObj, callback) {
                ...
                e.app[appName].connection[thunderrtcid].room[roomName] = {
                    apiField: {},
                    enteredOn: Date.now(),
                    gotListOn: Date.now(),
                    clientList: {},
                ...
                // Add thunderrtcid to room clientList
                e.app[appName].room[roomName].clientList[thunderrtcid] = {
                    enteredOn: Date.now(),
                    modifiedOn: Date.now(),
                    toConnection: e.app[appName].connection[thunderrtcid]
                };
         ...
         * @memberof    pub.appObj.connectionObj
         * @param       {string} roomName       Room name which uniquely identifies a room within an ThunderRTC application.
         * @param       {function(?Error, Object=)} callback Callback with error and object containing ThunderRTC connection room object.
         */
        connectionObj.room = function(roomName, callback) {
            if (_.isUndefined(e.app[appName].connection[thunderrtcid].room[roomName])) {
            ...
            /**
             * Returns the connection object to which the connection belongs. Note that unlike most ThunderRTC functions, this returns a value and does not use a callback.
             *
             * @memberof    pub.appObj.connectionObj.connectionRoomObj
             * @return      {Object}    The application object
             */
            ...
            /**
             * Returns the room object to which the connection belongs. Note that unlike most ThunderRTC functions, this returns a value and does not use a callback.
             *
             * @memberof    pub.appObj.connectionObj.connectionRoomObj
            ...
            /**
             * Returns the room name to which the connection belongs. Note that unlike most ThunderRTC functions, this returns a value and does not use a callback.
             *
             * @memberof    pub.appObj.connectionObj.connectionRoomObj
            ...
            connectionRoomObj.leaveRoom = function(next) {
                ...
                if (appObj.isRoomSync(roomName)){
                    ...
                    delete e.app[appName].room[roomName].clientList[thunderrtcid];
                }

                if (e.app[appName].connection[thunderrtcid]){
                    delete e.app[appName].connection[thunderrtcid].room[roomName];
                }
            ...
            connectionRoomObj.emitRoomDataDelta = function(isLeavingRoom, callback) {
                pub.util.logDebug("[" + appName + "][" + thunderrtcid + "] Room [" + roomName + "] Running func 'connectionRoomObj.emitRoomDataDelta'");
                if (!_.isFunction(callback)) {
                    callback = function(err, roomDataObj) {
                    };
                }
                ...
                connectionRoomObj.generateRoomDataDelta(isLeavingRoom, function(err, roomDataDelta) {
                    ...
                    for (var currentThunderrtcid in e.app[appName].room[roomName].clientList) {
                        // Anonymous wrapper to deliver arguments
                        (function(innerCurrentThunderrtcid, innerMsg){
                            connectionObj.getApp().connection(innerCurrentThunderrtcid, function(err, emitToConnectionObj) {
                                if (!err && innerCurrentThunderrtcid != thunderrtcid && emitToConnectionObj) {
            ...
            connectionRoomObj.generateRoomDataDelta = function(isLeavingRoom, callback) {
                pub.util.logDebug("[" + appName + "][" + thunderrtcid + "] Room [" + roomName + "] Running func 'connectionRoomObj.generateRoomDataDelta'");
                if (!_.isFunction(callback)) {
                ...
                if (isLeavingRoom) {
                    ...
                    roomDataDelta.clientListDelta.removeClient[thunderrtcid] = {"thunderrtcid": thunderrtcid};
                } else {
                    var connectionRoom = e.app[appName].connection[thunderrtcid].room[roomName];
                    roomDataDelta.clientListDelta.updateClient = {};
                    roomDataDelta.clientListDelta.updateClient[thunderrtcid] = {
                        "thunderrtcid": thunderrtcid,
                        "roomJoinTime": e.app[appName].connection[thunderrtcid].room[roomName].enteredOn,
                        "presence": e.app[appName].connection[thunderrtcid].presence
                    };

                    if (!_.isEmpty(e.app[appName].connection[thunderrtcid].room[roomName].apiField)) {
                        roomDataDelta.clientListDelta.updateClient[thunderrtcid].apiField = e.app[appName].connection[thunderrtcid].room[roomName].apiField;
                    }
                    if (e.app[appName].connection[thunderrtcid].username) {
                        roomDataDelta.clientListDelta.updateClient[thunderrtcid].username = e.app[appName].connection[thunderrtcid].username;
                    }
                }
            ...
            connectionRoomObj.setApiField = function(apiFieldObj, next) {
                ...
                e.app[appName].connection[thunderrtcid].room[roomName].apiField = pub.util.deepCopy(apiFieldObj);
                next(null);
            };
        ...
        connectionObj.removeConnection = function(next) {
            if (e.app[appName] && _.isObject(e.app[appName].connection) && e.app[appName].connection[thunderrtcid]) {
                e.app[appName].connection[thunderrtcid].isAuthenticated = false;
                // Remove link to connection from session in local storage
                if (e.app[appName].connection[thunderrtcid].toSession) {
                    delete e.app[appName].connection[thunderrtcid].toSession.toConnection[thunderrtcid];
                }

                // Remove connection from local storage
                delete e.app[appName].connection[thunderrtcid];
            }
            next(null);
        };
        ...
        if (e.app[appName].connection[thunderrtcid].toSession) {
            appObj.session(e.app[appName].connection[thunderrtcid].toSession.thunderrtcsid, function(err, sessionObj) {
                if (err) {
                    callback(err);
                    return;
                }
     ...
     * @memberof    pub.appObj
     * @param       {string}    thunderrtcid   ThunderRTC unique identifier for a socket connection.
     * @param       {function(?Error, Object=)} callback Callback with error and object containing ThunderRTC connection object (same as calling connection(thunderrtcid))
     */
    appObj.createConnection = function(thunderrtcid, callback) {
        if (!thunderrtcid || !appObj.getOption("thunderrtcidRegExp").test(thunderrtcid)) {
            pub.util.logWarning("Can not create connection with improper name: '" + thunderrtcid + "'");
            callback(new pub.util.ConnectionWarning("Can not create connection with improper name: '" + thunderrtcid + "'"));
            return;
        }

        if (e.app[appName].connection[thunderrtcid]) {
            pub.util.logWarning("Can not create connection which already exists: '" + thunderrtcid + "'");
            callback(new pub.util.ConnectionWarning("Can not create connection which already exists: '" + thunderrtcid + "'"));
            return;
        }

        // Set the connection structure with some default values
        e.app[appName].connection[thunderrtcid] = {
            thunderrtcid: thunderrtcid,
            connectOn: Date.now(),
        ...
        appObj.connection(thunderrtcid, function(err, connectionObj) {
            if (err) {
                callback(err);
                return;
            }
     ...
     * @param       {function(?Error, Object=)} callback Callback with error and object containing ThunderRTC room object (same as calling appObj.room(roomName))
     */
    ...
    /**
     * Creates a new session with a provided thunderrtcsid
     *
     * @memberof    pub.appObj
     * @param       {string}    thunderrtcsid  ThunderRTC Session Identifier. Must be formatted according to "thunderrtcsidRegExp" option.
     * @param       {function(?Error, Object=)} callback Callback with error and object containing ThunderRTC session object (same as calling session(thunderrtcsid))
     */
    appObj.createSession = function(thunderrtcsid, callback) {
        pub.util.logDebug("[" + appObj.getAppName() + "] Creating session [" + thunderrtcsid + "]");

        if (!thunderrtcsid || !appObj.getOption("thunderrtcsidRegExp").test(thunderrtcsid)) {
            pub.util.logWarning("Can not create session with improper name [" + thunderrtcsid + "]");
            callback(new pub.util.ConnectionWarning("Can not create session with improper name [" + thunderrtcsid + "]"));
            return;
        }

        if (e.app[appName].session[thunderrtcsid]) {
            pub.util.logWarning("Can not create session which already exists [" + thunderrtcsid + "]");
            callback(new pub.util.ConnectionWarning("Can not create session which already exists [" + thunderrtcsid + "]"));
            return;
        }

        // Set the session structure with some default values
        e.app[appName].session[thunderrtcsid] = {
            "thunderrtcsid": thunderrtcsid,
            "startOn": Date.now(),
            "toConnection":{},
            "field": {}
        };

        appObj.session(thunderrtcsid, callback);
    };
     ...
     */
    appObj.isSession = function(thunderrtcsid, callback) {
        callback(null, (e.app[appName].session[thunderrtcsid] ? true : false));
    };
     ...
     * @param       {function(?Error, Object=)} callback Callback with error and object containing ThunderRTC group object.
     */
    appObj.group = function(groupName, callback) {
         ...
         *
         * @ignore
         * @param {function(?Error, Array.<string>)} callback Callback with error and array containing all thunderrtcids.
         */
        groupObj.getConnections = function(callback) {
            var connectedThunderrtcidArray = [];
            for (var key in e.app[appName].group[groupName].clientList) {
                connectedThunderrtcidArray.push(key);
            }
            callback(null, connectedThunderrtcidArray);
        };
     ...
     * @memberof    pub.appObj
     * @param       {string} roomName       Room name which uniquely identifies a room within an ThunderRTC application.
     * @param       {function(?Error, Object=)} callback Callback with error and object containing ThunderRTC room object.
     */
    appObj.room = function(roomName, callback) {
        ...
        /**
         * ThunderRTC Room Object. Contains methods for handling a specific room including determining which connections have joined.
         *
         * @class       roomObj
         * @memberof    pub.appObj
         */
        ...
        /**
         * Returns the application object to which the room belongs. Note that unlike most ThunderRTC functions, this returns a value and does not use a callback.
         *
         * @memberof    pub.appObj.roomObj
         * @return      {Object}    The application object
         */
        ...
        /**
         * Returns the application name for the application to which the room belongs. Note that unlike most ThunderRTC functions, this returns a value and does not use a callback.
         *
         * @memberof    pub.appObj.roomObj
         * @return      {string}    The application name
         */
        ...
        /**
         * Returns the room name for the current room. Note that unlike most ThunderRTC functions, this returns a value and does not use a callback.
         *
         * @memberof    pub.appObj.roomObj
         * @return      {string}    The room name
         */
         ...
         * @ignore 
         */
        roomObj.emitRoomDataFieldUpdate = function(skipThunderrtcid, next) {
            roomObj.getFields(true, function(err, fieldObj) {
                ...
                async.each(
                        ...
                        function(currentThunderrtcid, asyncCallback) {

                            // Skip a given thunderrtcid?
                            if (skipThunderrtcid && (skipThunderrtcid == currentThunderrtcid)) {
                                asyncCallback(null);
                                return;
                            }

                            // Retrieve a connection object, then send the roomData message.
                            appObj.connection(currentThunderrtcid, function(err, targetConnectionObj) {
                                if (err || !_.isObject(targetConnectionObj)) {
                                    pub.util.logDebug("[" + currentThunderrtcid + "] Could not get connection object to send room data field update. Client may have disconnected.");
                                    asyncCallback(null);
                                    return;
                                }
                                pub.events.emit("emitThunderrtcCmd", targetConnectionObj, "roomData", outgoingMsg, function(msg) {
                                }, function(err) {
        ...
        /**
         * Incomplete function for setting an thunderrtcid as being a client in a room.
         *
         * @memberof    pub.appObj.roomObj
         * @param       {string}    thunderrtcid   ThunderRTC unique identifier for a socket connection.
         * @param       {nextCallback} next     A success callback of form next(err).
         * @ignore
         */
        roomObj.setConnection = function(thunderrtcid, next) {
            if (!appObj.isRoomSync(roomName)) {
                pub.util.logWarning("Attempt to request non-existent room name: '" + roomName + "'");
                next(new pub.util.ApplicationWarning("Attempt to request non-existent room name: '" + roomName + "'"));
                return;
            ...
            e.app[appName].room[roomName].clientList[thunderrtcid] = {enteredOn: Date.now()};
            next(null);
        };
        ...
        /**
         * Returns an array containing the thunderrtcids of all connected clients within the room.
         *
         * @memberof    pub.appObj.roomObj
         * @param {function(?Error, Array.<string>=)} callback Callback with error and array containing all thunderrtcids.
         */
        ...
        /**
         * Returns the connectionObj for a given thunderrtcid, but only if it is currently a client in the room
         *
         * @memberof    pub.appObj.roomObj
         * @param       {string}    thunderrtcid   ThunderRTC unique identifier for a socket connection.
         * @param {function(?Error, Object=)} callback Callback with error and connectionObj.
         */
        roomObj.getConnectionWithThunderrtcid = function(thunderrtcid, callback) {
            if (!appObj.isRoomSync(roomName)) {
                pub.util.logWarning("Attempt to request non-existent room name: '" + roomName + "'");
                callback(new pub.util.ApplicationWarning("Attempt to request non-existent room name: '" + roomName + "'"));
                return;
            }
            if (e.app[appName].room[roomName].clientList[thunderrtcid]){
                appObj.connection(thunderrtcid, function(err, connectionObj) {
                    if (err) {
                        callback(new pub.util.ConnectionWarning("Can not find connection [" + thunderrtcid + "] in room."));
                        return;
                    }
        ...
        roomObj.getConnectionObjects = function(callback) {
            ...
            async.each(Object.keys(e.app[appName].room[roomName].clientList),
                    function(currentThunderrtcid, asyncCallback) {
                        appObj.connection(currentThunderrtcid, function(err, connectionObj) {
                            if (err) {
    ...
    /**
     * NOT YET IMPLEMENTED - Gets session object for a given thunderrtcsid. Returns null if session not found.
     * The returned session object includes functions for managing session fields.
     *
     * @memberof    pub.appObj
     * @param       {string}    thunderrtcsid      ThunderRTC session identifier
     * @param       {function(?Error, Object=)} callback Callback with error and object containing ThunderRTC session object.
     */
    appObj.session = function(thunderrtcsid, callback) {

        if (!e.app[appName].session[thunderrtcsid]) {
            pub.util.logWarning("Attempt to request non-existent thunderrtcsid: '" + thunderrtcsid + "'");
            callback(new pub.util.ApplicationWarning("Attempt to request non-existent thunderrtcsid: '" + thunderrtcsid + "'"));
            return;
        }

        /**
         * The primary method for interfacing with an ThunderRTC session.
         *
         * @class       sessionObj
         * @memberof    pub.appObj
         */
        ...
        /**
         * Returns the application object to which the session belongs. Note that unlike most ThunderRTC functions, this returns a value and does not use a callback.
         *
         * @memberof    pub.appObj.sessionObj
         * @return      {Object}    The application object
         */
        ...
        /**
         * Returns the application name for the application to which the session belongs. Note that unlike most ThunderRTC functions, this returns a value and does not use a callback.
         *
         * @memberof    pub.appObj.sessionObj
         * @return      {string}    The application name
         */
        ...
        /**
         * Returns the thunderrtcsid for the session.  Note that unlike most ThunderRTC functions, this returns a value and does not use a callback.
         *
         * @memberof    pub.appObj.sessionObj
         * @return      {string}    Returns the thunderrtcsid, which is the ThunderRTC unique identifier for a session.
         */
        sessionObj.getThunderrtcsid = function() {
            return thunderrtcsid;
        };

        /**
         * Returns the thunderrtcsid for the session. Old SessionKey name kept for transition purposes. Use getThunderrtcsid();
         * 
         * @memberof    pub.appObj.sessionObj
         * @ignore
         */
        sessionObj.getSessionKey = sessionObj.getThunderrtcsid;
        ...
        sessionObj.getField = function(fieldName, callback) {
            if (!e.app[appName].session[thunderrtcsid].field[fieldName]) {
            ...
            callback(null, pub.util.deepCopy(e.app[appName].session[thunderrtcsid].field[fieldName]));
        };
        ...
        sessionObj.getFieldSync = function(fieldName) {
            ...
            return pub.util.deepCopy(e.app[appName].session[thunderrtcsid].field[fieldName]);
        };
        ...
        sessionObj.getFieldValueSync = function(fieldName) {
            ...
            return pub.util.deepCopy(e.app[appName].session[thunderrtcsid].field[fieldName].fieldValue);
        };
        ...
        sessionObj.getFields = function(limitToIsShared, callback) {
            var fieldObj = {};
            for (var fieldName in e.app[appName].session[thunderrtcsid].field) {
                if (!limitToIsShared || e.app[appName].session[thunderrtcsid].field[fieldName].fieldOption.isShared) {
                    fieldObj[fieldName] = {
                        fieldName: fieldName,
                        fieldValue: pub.util.deepCopy(e.app[appName].session[thunderrtcsid].field[fieldName].fieldValue)
                    };
                }
            }
            callback(null, fieldObj);
        ...
        sessionObj.setField = function(fieldName, fieldValue, fieldOption, next) {
            pub.util.logDebug("[" + appName + "] Session [" + thunderrtcsid + "] - Setting field [" + fieldName + "]", fieldValue);
            if (!_.isFunction(next)) {
                next = pub.util.nextToNowhere;
            }
            ...
            e.app[appName].session[thunderrtcsid].field[fieldName] = {
                fieldName: fieldName,
                fieldValue: fieldValue,
                fieldOption: {isShared: ((_.isObject(fieldOption) && fieldOption.isShared) ? true : false)}
            };
        ...
        sessionObj.emitSessionDataFieldUpdate = function(next) {
            sessionObj.getFields(true, function(err, fieldObj) {
                ...
                async.each(
                    Object.keys(e.app[appName].session[thunderrtcsid].toConnection),
                    function(currentThunderrtcid, asyncCallback) {

                        // Retrieve a connection object, then send the sessionData message.
                        appObj.connection(currentThunderrtcid, function(err, targetConnectionObj) {
                            if (err || !_.isObject(targetConnectionObj)) {
                                pub.util.logDebug("[" + currentThunderrtcid + "] Could not get connection object to send session data field update. Client may have disconnected.");
                                asyncCallback(null);
                                return;
                            }

                            // Emit sessionData thunderrtcCmd to each connection
                            pub.events.emit("emitThunderrtcCmd", targetConnectionObj, "sessionData", outgoingMsg, function(msg) {
                            }, function(err) {
                                // Ignore errors if unable to send to a socket. 
                                asyncCallback(null);
                            });
