ThunderRTC Server: API to/from Server Messages
===========================================

ThunderRTC socket messages are sent via socket.io using three custom emit types:

 - **thunderrtcAuth** - Initial negotiation and authentication
 - **thunderrtcCmd** - All standard ThunderRTC communications including WebRTC messages
 - **thunderrtcMsg** - Custom application specific messages
...
"thunderrtcAuth" is the socket.io emit type which ThunderRTC expects to initialize (or re-initialize) a connection. No other messages are handled by ThunderRTC until a connection is authenticated.
...
**msgData Fields:**
 ...
 - **thunderrtcsid** (optional) The ThunderRTC session ID which should be available in the browser cookie variables.
...
The thunderrtcCmd is the core socket.io emit type which ThunderRTC uses to send and receive commands.
...
**msgData Fields:**
 ...
 - **roomJoin** (required) Map of room names
   ...
   - **roomParameter** (optional) A map(dictionary) object with key/value pairs. The values can be any JSONable object. This field is not currently looked at by ThunderRTC, however it is available for custom server applications. May be used for room options or authentication needs.
...
**msgData Fields:**
 ...
 - **setUserCfg**
   - **p2pList** (optional) Map of all connections with their statistics. The map key is the thunderrtcid's. Unlike userSettings and apiField, this field must contain all current connections. Any connections not mentioned will be removed.
...
Initiates an authenticated ThunderRTC application. Note this may be sent multiple times in a session upon configuration changes. The API should reset application, room, and list data.
...
**msgData Fields:**
 - **roomData** (required) Map of room names
   ...
   - **clientList** (optional) Map of thunderrtcid's for users online in the same room. If present, this should overrule the current list in memory.
     - **thunderrtcid** (required) Matches map key
     - **username** (optional)
   ...
   - **clientListDelta** (optional)
     - **updateClient** (optional) Map of thunderrtcids to update. Will contain same fields as 'clientList'
     - **removeClient** (optional) Map of thunderrtcids to remove from the client list.
...
**msgData Fields:**
 - **sessionData** (required) Object holding session information
   - **thunderrtcsid** (required) Session identifier
...
"thunderrtcMsg" is the socket.io emit type which ThunderRTC uses for custom application level messages.
...
**Fields:**
 ...
 - **targetThunderrtcid** (optional) - If present, will attempt to forward the message to the specific thunderrtcid.
 - **targetGroup** (optional) - If present, will attempt to forward the message to all clients in a specific group. Can work in conjunction with targetThunderrtcid and targetRoom to further restrict the recipient.
 - **targetRoom** (optional) - If present, will attempt to forward the message to all clients in a specific room. Can work in conjunction with targetThunderrtcid and targetGroup to further restrict the recipient.
...
**Fields:**
 ...
 - **msgType** (required) - String containing the message type.
 - **msgData** (optional) - Can contain any JSON compatible object or primitive type.
 - **senderThunderrtcid** (optional) - If present, indicates the message was sent by a specific user.
 - **targetThunderrtcid** (optional) - If present, indicates the message was intended only for this specific thunderrtcid.
 - **targetGroup** (optional) - If present, indicates the message was intended only for clients in this specific group. Can work in conjunction with targetThunderrtcid and targetRoom to further restrict the recipient.
 - **targetRoom** (optional) - If present, indicates the message was intended only for clients in this specific room. Can work in conjunction with targetThunderrtcid and targetGroup to further restrict the recipient.
