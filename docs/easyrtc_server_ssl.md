ThunderRTC Server: Using SSL
=========================

Being a signaling server, ThunderRTC sends many message in plain text between the client and server. It is a good idea to connect to ThunderRTC via SSL.
...
---------------------------------
    ...
    var rtc = thunderrtc.listen(httpApp, socketServer);
    

Using an SSL Gateway or Load Balancing server:
----------------------------------------------

For performance, load balancing, or firewall reasons you may wish to use an SSL gateway or load balancer. These often entail the gateway handling SSL, then routing requests to an ThunderRTC server in a virtual private network. The gateway should be set to include the X-Forwarded-For http header. There are many solutions for doing this, each with their own instructions.
