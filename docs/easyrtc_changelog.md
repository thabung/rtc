...
New Features:
 ...
 peer is still in any room. This is actually exposing an internal function.
  - JS Client - New function getNameOfRemoteStream() maps a remote stream 
   to the name it was assigned by the peer.
  - JS Client - New function setIceCandidateFilter() provides a hook to filter
   ice candidates on the fly. In theory, this should make it easier to test 
   turn servers, just filter out the candidates that don't reference the 
   turn server ip address.
  - JS Client - New function register3rdPartyLocalMediaStreams is used to 
register local media streams that were created external to ThunderRTC (ie, by
other frameworks).
  - JS Client - Stream names are now faked on Firefox. This is not a real 
...
after authenticating.  This was done to make it more clear about when it was legal to call it.
  - JS Client - Added code to clear room data and your own thunderrtcid on disconnecting from a server.  This was done to reduce confusion for applications that disconnected and reconnected.
  ...
  - JS Client - Added some support for the event "negotiationneeded". This 
should be transparent to developers. With thanks to Christophe Eyrignoux.
  - JS Client - Added code to remove information about a room after it has 
been exited.
  - JS Client - Added a connection options parameter to setSocketUrl. The 
default connection options now include a connect timeout of 10 seconds and
a directive to always allocated a new connection (rather than reusing an old
one). This was a fix for issue#107.
  - JS Client - Added some extra controls to the demo_audio_video demo.
  - JS Client - Added an extra callback parameter to the addStreamToCall method
...
Fixes:
  ...
  - JS Client - Added aggregating timers to compress a bunch of roomOccupant
 events that occur in a short span into a single event so that UI don't need to
 update as frequently as they would otherwise.
  - JS Client - If you call getPeerStatistics for a peer that isn't associated
with a peer connection, you'll get a more useful object being passed to
your callback now.
...
Changes:
 ...
 - JS Client - an onStreamClosed listener gets passed an thunderrtcid plus the stream and the stream name.
...
Changes:
 ...
 - Documentation - New documentation for upgrading ThunderRTC using NPM. (issue #43)

Fixes:

 - JS Client - Updated data channel support for Firefox so that it interops with Chrome again. 
 - JS Client - Fixed the thunderrtc.supportsDataChannel methods check for Android browsers. Data channels aren't currently supported for Android by ThunderRTC.
...
New Features:
 ...
 - JS Client - Added thunderrtc.getRoomApiField() convenience method to get a peers API fields.
...
Fixes:
 ...
 - JS Client - A couple type-o fixes in thunderrtc.supportsDataChannels()
 - JS Client - Fixed type-o in thunderrtc.connect() causing bad reference to listener for signal failure. See issue #32 
 - JS Client - Fixed reference to error code in thunderrtc.connect()
 ...
 - JS Client - Fixed bug in thunderrtc.sendServerMessage when debug was enabled.
...
Changes:
 ...
 - JS Client - Renamed thunderrtc.usernamePattern to thunderrtc.usernameRegExp to align with server option name. Set the default value to align with default server option value.
...
New Features:
 ...
 - JS Client - New convenience function to return an array of thunderrtcid's which match a username. thunderrtc.usernameToIds()
 - Server - New application level function for determining if an thunderrtcid is currently connected. appObj.isConnected()
 - Documentation - New guide for installing ThunderRTC alongside another server
 - Documentation - New guide for running ThunderRTC with SSL
 - Documentation - New guide for using rooms (still in progress)

Changes:

 - JS Client - thunderrtc_closeicon uses an inline svg in the thunderrtc.css file instead of an external svg.
 - JS Client - thunderrtc.easyApp now renders remote video object invisible on stream close
...
Fixes:
 ...
 - JS Client - Handle a possibility of the lastLoggedInList not having a room entry before adding an thunderrtcid to it (Related to issue #14)
 - Demos - Screen share and rooms demo now properly use thunderrtc.setUsername()
 - Server - Now disconnects client if an thunderrtcAuth message is received when they are already logged in.
 - Server - Message verification fixes for thunderrtcsid, and application names
...
Fixes:
 ...
 - JS Client - Fixed a string split issue when sending thunderrtcsid value
...
New Features:
 ...
 - Server - thunderrtc_default_event_listeners.js has been improved by the addition of several new listeners. JSDoc's have been written for each default listener.
...
New Features:
 ...
 - JS Client - thunderrtc_ft.js - A separate official ThunderRTC module for handling file transfers. See the updated file sharing demo for an example.
...
New Features:
 ...
 - Server/API - Rooms. An ThunderRTC application can have multiple rooms. A user can be in one or more rooms at the same time. Users can only see other users in the same room. If no room is provided, connections will be entered into a room named `default`.
 ...
 - Server - Many new server events. This is intended to be the new primary way for developers to interact with ThunderRTC server.
 - Documentation - New documentation for internal ThunderRTC command messages which highlight how the API and server communicate to each other.
...
Changes:
 ...
 - Server/API - Initial authentication and application setup now handled by a separate socket.io message type called 'thunderrtcAuth'. This allows us to easily ignore other messages until a client has authenticated.
 - Server - Better incoming message validation. Now it's handled once right after the message is received.
 - Documentation - While the ThunderRTC logo remains, when in text form, ThunderRTC will have a capitol "E", which should make writing about it in sentences easier.
...
New Features:
 ...
 - JS Client - new thunderrtc.setPeerListener() function. Sets a listener for data sent from another client. Replacement for set data listener.
 - JS Client - new thunderrtc.setServerListener() function. Listens for messages from the server which are not from another peer.

Changes:

 - JS Client - Implementing methods used in Google's adapter.js for cross browser support
 - Server/API - Renamed easyRTCcmd socket message type to thunderrtcCmd. Should have no outside effect.
 - Server - Moved API files to the /api/ folder thus cleaning up the /static/. API files are publicly linked using /thunderrtc/thunderrtc.js and /thunderrtc/thunderrtc.css. For transitional purposes, the old public file locations are still accessible.
...
New Features:
 ...
 - Documentation - Moved the client API documentation from mark-down format to jsDoc and added inline examples. Check out the thunderrtc.html file in the docs directory. The thunderrtcjs.html file is a helper file that shouldn't be looked at directly.
...
New Features:
 ...
 - JS Client - Added more debugging output and provided a means to control it through the thunderrtc.debugPrinter variable and thunderrtc.enableDebug function.
 - JS Client - Added code to log application state (WRT webrtc) to the server.
 - JS Client - New function setSocketUrl() to point to web socket server. Allows website to be hosted using a seperate server (suchs as Apache). The default remains for the ThunderRTC server to function as both the web and socket server.
 - JS Client - Support for hanging up on calls still being set up - on the initiating side by extending the thunderrtc.hangup function, and on the receiving side by adding the thunderrtc.setCallCancelled callback setter.
 - JS Client - Added thunderrtc.getConnectStatus function to get the state of a connection to a peer.
...
Changes:
 ...
 - JS Client - The callSuccessCB argument to thunderrtc.call now has a second argument, which can be either 'audiovideo' or 'datachannel'. The callSuccessCB function may be get called twice if the peer connection is using data channels as well as audio or video.
 - JS Client - Fixed thunderrtc.connect so that you can reconnect after calling disconnect.
...
New Features:
 ...
 - JS Client - powered_by_thunderrtc.png image. Please use it to promote the project.
