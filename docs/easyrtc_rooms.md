...
Joining a room is handled using the joinRoom function. This can be run before or after `thunderrtc.connect()` or `thunderrtc.easyApp()`. It may be called multiple times to be in multiple rooms simultaneously. It may be called before or after connecting to the server.

Note: the successCB and failureDB will only be called if you are already connected to the server.

**Format**

`thunderrtc.joinRoom(roomName, roomParameters, successCB, failureCB)`
